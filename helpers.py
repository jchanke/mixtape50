...
# Adapated from
# https://github.com/Deffro/statify/blob/dd15a6e70428bd36ecddb5d4a8ac3d82b85c9339/code/server.py#L553
...
def search_message(
    message: str,
    max_search_length: int = 10,
    query_lookup: Dict[str, list] = dict(),
    failed_queries: set = set(),
) -> List[Union[list, Any]]:
    """
    >>> search_message("Hello, world!", max_search_length = 10)
    ['hello', 'world']
    ...
    This function is instrumented with an announcer object to send messages to
    ...
    for i in range(query_length):
        ...
        for search_function in search_functions:
            ...
            if prefix_results:
                ...
                print(
                    f"[DEBUG] Try: {prefix} in {
                        search_function.__name__.replace(
                            'search_', '')}"
                )

                # In announcer: replace prefix, add each track in
                # prefix_results
                announcer.announce(format_sse(event="drop", data=prefix))
                for track in map(lambda tracks: tracks[0]["name"], prefix_results):
                    announcer.announce(
                        format_sse(
                            event="add",
                            data=remove_punctuation(clean_title(track.casefold())),
                        )
                    )
                    time.sleep(0.1)

                # Base case: if prefix is whole message, suffix == "", so we
                # should just return prefix
                if suffix == "":
                    print(f"[DEBUG] All done!")
                    announcer.announce(format_sse(event="lock in"))
                    return prefix_results

                # Recursive case: make sure suffix it can be split into songs
                # as well
                suffix_results = search_message(
                    suffix,
...
def search_spotipy(query: str, query_lookup: Dict[str, list]) -> list:
    ...
    results = [
        ...
        for item in results
        if remove_punctuation(clean_title(item["name"].casefold()))
